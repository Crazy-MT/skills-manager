...
export function getCurrentDescriptionLocale() {
    const stateFile = path.join(os.homedir(), '.skills-manager', 'tui-state.json');
    try {
        const state = JSON.parse(fs.readFileSync(stateFile, 'utf8'));
        if (state.descriptionLocale?.trim())
            return normalizeLocaleForCache(state.descriptionLocale);
    }
    catch {
...
export function resolveDescriptionLocale(metadata, description, fallback = 'en') {
...
export function getCachedTranslation(skillID, sourceText, sourceLocale = 'en', targetLocale = getCurrentDescriptionLocale()) {
    if (!sourceText.trim())
        return undefined;
    ...
    if (!entry || entry.translatorVersion !== TRANSLATOR_VERSION)
        return undefined;
    return entry.translatedText;
}
export function buildLocalizedDescription(skillID, baseDescription, baseDescriptionLocale = 'en') {
...
function cacheKey(skillID, sourceText, sourceLocale, targetLocale) {
    void skillID;
...
function sourceTextHash(sourceText) {
    return crypto.createHash('sha256').update(sourceText).digest('hex');
}
function normalizeExplicitLocale(raw) {
...
function normalizeLocaleIdentifier(locale) {
...
function normalizeLocaleForCache(locale) {
...
function detectCjkLocale(text) {
    ...
    for (const char of text) {
        ...
        if ((codePoint >= 0x3040 && codePoint <= 0x30ff) || (codePoint >= 0x31f0 && codePoint <= 0x31ff)) {
            ...
            hanCount += 1;
            if (SIMPLIFIED_CHINESE_CHARS.has(char))
                simplifiedCount += 1;
    ...
    if (cjkCount < 2)
        return undefined;
    if (cjkCount / Math.max(letterCount, 1) < 0.2)
        return undefined;
...
function chineseScriptCode(locale) {
    ...
    for (const part of rest) {
        ...
        if (lower === 'hant')
            return 'Hant';
        if (['cn', 'sg', 'my'].includes(lower))
            return 'Hans';
        if (['tw', 'hk', 'mo'].includes(lower))
            return 'Hant';
    }
    return undefined;
}
